...
def render(config_path="config.yaml"):
    configs = {}
    ...
    with st.sidebar:
        try:
                ...
                for sidebar_item in yaml_content["sidebar"]:
                    ...
                    if "enable_when" not in sidebar_item or (
                        ...
                        if sidebar_item["enable_when"]["condition"] == "is":
                            if (
                                configs[sidebar_item["enable_when"]["variable_name"]]
                                == sidebar_item["enable_when"]["value"]
                            ):
                                enable = True
                        elif (
                            sidebar_item["enable_when"]["condition"] == "is_not"
                        ):
                            if (
                                configs[sidebar_item["enable_when"]["variable_name"]]
                                != sidebar_item["enable_when"]["value"]
                            ...
                            sidebar_item["enable_when"]["condition"] == "not_in"
                        ):
                            if (
                                configs[sidebar_item["enable_when"]["variable_name"]]
            ...
            st.error(f"Error parsing YAML file: {error}")

    return configs

if __name__ == "__main__":
    render(config_path="C:/Users/mgm_c/OneDrive/Desktop/QuickStartML-main/config.yaml")  # Provide the correct path here
